...
def run_script_nextflow(runDate,path_to_nextflow,download_path,run_id,run_datas,nextflow_workdir):
    ...
    #first flow
    subprocess.run(". $CONDA_PREFIX/home/ssh_user/mambaforge/etc/profile.d/conda.sh && conda activate WasteWater && nextflow run "+path_to_nextflow+" --in '"+path_to_bam+"/*.bam' --run_date "+runDate+" -w "+nextflow_workdir+" && source deactivate",shell=True)

    #second flow
    subprocess.run(". $CONDA_PREFIX/home/ssh_user/mambaforge/etc/profile.d/conda.sh && conda activate WasteWater && nextflow run "+path_to_nextflow+" -entry final_step --run_date "+runDate+" -w "+nextflow_workdir+" && source deactivate",shell=True)
